...
class ClipboardImage(BaseSchema):
    ...
    This model contains metadata and base64-encoded image data
    for images stored or retrieved from the clipboard.
    """
    ...
    def validate_base64(cls, value: str) -> str:
        """Validate that the ``data`` field contains valid Base64.

        :param value: Base64 string to validate.
        :type value: str
        :return: The validated Base64 string.
        :rtype: str
        :raises ValueError: If the value is not valid Base64.
        """
    ...
    def accept_list(cls, v: list[Any] | dict[str, Any]) -> dict[str, Any] | list[Any]:
        """Allow deserialization from a Rust-style array ``[width, height, data]``.

        :param v: Input value from the serializer.
        :type v: list[Any] | dict[str, Any]
        :return: Converted dictionary with keys ``width``, ``height`` and ``data``,
                 or the original input if not applicable.
        :rtype: dict[str, Any] | list[Any]
        """
        if isinstance(v, list) and len(v) == 3:
            return {"width": v[0], "height": v[1], "data": v[2]}
        return v
